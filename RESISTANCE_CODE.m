...
for i = 0.2:0.2:10
    ...
    y1(counter) = plotFEMMWindingPowerLosses(i);
    y2(counter) = plotAnalyticalWindingPowerLosses(i);
...
function power = plotFEMMWindingPowerLosses(amps)
    mi_analyze();
    mi_loadsolution();
    CP = mo_getcircuitproperties('winding_1');
    resistanceFEMM = CP(2)/CP(1);
    resistanceFEMM = 0.0107;
    power = amps^2 * resistanceFEMM;
end

function power = plotAnalyticalWindingPowerLosses(amps)

...
%function to move armature and ensure it doesn't move beyond core boundary
...
%calculates inductances for both analytical cases and numerical linear case.
function linInd = linear_inductances(armPos)
    ...
    %calculate analytical inductance, no fringing
    Lanalytical = (100^2)/Rtot;
    %calculate analytical inductance, fringing
    ...
    % circuit. Output array of calculated inductances.
    linInd = [armPos 2*Lanalytical ...
                         2*Lanalyticalfringe 2*Lnumericallinear];
end

%calculates inductance for non-linear core material.
function [nonLinInd, Ec] = nonlinear_inductances()
    ...
    CP = mo_getcircuitproperties('winding_1');
    mo_groupselectblock();
    %extract co-energy from FEMM
    Ec = mo_blockintegral(17);
    nonLinInd = 2*CP(3)/CP(1);
end

%sets core material to linear
...
%sets core material to non-linear
